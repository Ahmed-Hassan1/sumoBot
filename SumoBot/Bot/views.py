...
def testing(request):
    ...
    for check_balance in acc_balance:
        if check_balance["asset"] == "USDT":
            ...
            #USDT INPUT PER TRADE
            quantity+=int(float(USDT_QUANT/currentPrice))
    print(balance)
    print(quantity)
...
@csrf_exempt
def bot2nd(request):
    botInfo=SecondBot.objects.first()
    ...
    try:
        ...
        for check_balance in acc_balance:
            if check_balance["asset"] == "USDT":
                usdt_balance = check_balance["balance"]
                balance+=float(usdt_balance)
        if botInfo.currentDay != datetime.now(timezone.utc).date():
            botInfo.currentDay=datetime.now(timezone.utc).date()
            botInfo.startingDayBalance=balance
            botInfo.dailyProfit=balance*profitPerc/100
            botInfo.save()
            logger.info("NEW DAY")
            logger.info(balance)
        print(str(balance)+"   "+str(botInfo.startingDayBalance+botInfo.dailyProfit))
        if balance>=botInfo.startingDayBalance+botInfo.dailyProfit:
            return JsonResponse({"PnL":balance},safe=False)
    except Exception as e:
            ...
            if openTrades==0:
                ...
                for check_balance in acc_balance:
                    if check_balance["asset"] == "USDT":
                        ...
                        info=client.futures_symbol_ticker(symbol=symbol)
                        currentPrice=float(info['price'])
                        usdt=int(float(usdt_balance))*PercPerTrade/100
                        #USDT INPUT PER TRADE
                        quantity+=int(float(usdt/currentPrice))
                logger.info(quantity)
                #change Margin Type
                try:
                    ...
                    logger.info("1"+e.__str__())
                #change leverage INPUT LEVERAGE
                try:
                    info = client.futures_change_leverage(symbol=symbol,leverage=1)
                    logger.info(info)
                except Exception as e:
                ...
                if balance>=100:
                    try:
                        ...
                        if side=="Buy":
                            ...
                            data=client.futures_create_order(symbol=symbol, type=Client.FUTURE_ORDER_TYPE_MARKET, side=Client.SIDE_SELL, quantity=quantity)
                    ...
                    try: 
                        #STOP
                        if side=="Buy":
                            data=client.futures_create_order(symbol=symbol, type=Client.FUTURE_ORDER_TYPE_STOP, side=Client.SIDE_SELL, quantity=quantity,stopprice=stop,price=stop)
                            logger.info("Stop")
                            logger.info(data)
                            logger.info(quantity)

                        elif side=="Sell":
                            data=client.futures_create_order(symbol=symbol, type=Client.FUTURE_ORDER_TYPE_STOP, side=Client.SIDE_BUY, quantity=quantity,stopprice=stop,price=stop)
                            logger.info("Stop")
                            logger.info(data)
                            logger.info(quantity)
                    ...
                    try:
                        #Profit
                        if side=="Buy":
                            data=client.futures_create_order(symbol=symbol, type=Client.FUTURE_ORDER_TYPE_LIMIT, side=Client.SIDE_SELL, quantity=quantity,price=target,limitprice=target,timeinforce=Client.TIME_IN_FORCE_GTC)
                            ...
                            data=client.futures_create_order(symbol=symbol, type=Client.FUTURE_ORDER_TYPE_LIMIT, side=Client.SIDE_BUY, quantity=quantity,price=target,limitprice=target,timeinforce=Client.TIME_IN_FORCE_GTC)
...
def Percision(numb,precision):
    return "{:0.0{}f}".format(float(numb), precision)